...
class Harness:
    ...
    def _wire_strip_tb(self, cable: Cable, pad: bool) -> List[str]:
        ...
        for i, (connection_color, wirelabel) in enumerate(
            ...
            strip.append(f'    <td port="w{i}" cellpadding="0" width="{2 * len(bgcolors)}">')
            strip.append('     <table cellspacing="0" cellborder="0" border="0"><tr>')
            for bgcolor in bgcolors:
                strip.append(f'      <td width="2" bgcolor="{bgcolor if bgcolor != "" else wv_colors.default_color}" border="0"></td>')
        ...
        if cable.shield:
            ...
            if isinstance(cable.shield, str):
                ...
                attributes = (
                    f'width="6" bgcolor="{shield_color_hex}" border="2" sides="lr"'
                )
            else:
                # shield is shown as a thin black wire
                attributes = f'width="2" bgcolor="#000000" border="0"'
            # fmt: off
    ...
    def _compass(self) -> Tuple[str, str]:
        ...
        Wires are drawn as multi-colour parallel edges between ports. If the
        compass points face across the flow direction -- east/west under a
        top-to-bottom layout -- every edge has to loop sideways to reach its
        port, and the parallel colour stripes splay apart around that loop
        instead of running side by side. The wire then renders as a lens or
        squashed oval rather than a line.
        """
        return ("e", "w") if self.options.rankdir == "LR" else ("s", "n")
    ...
    def create_graph(self) -> Graph:
        ...
        for mate in self.mates:
            ...
            if isinstance(mate, MatePin) and to_connector.style != "simple":
                ...
                to_port_str = ""
            out_c, in_c = self._compass
            code_from = f"{mate.from_name}{from_port_str}:{out_c}"
